...
## About This Tutorial
<div style= "float:right;position: relative; top: 10px; height: 200px; width: 200px">
![](rdatatable.png)
</div>

This document was created by [Bijan Seyednasrollah](http://bnasr.github.io/) and it is partially adopted from a course by [Matt Dowle](https://twitter.com/MattDowle) and [Arun Srinivasan](https://twitter.com/arun_sriniv) at [DataCamp](https://learn.datacamp.com/courses/data-manipulation-in-r-with-datatable). You can get more information about the course and other resources at the bottom of this page. 
 
 If you follow the tutorial, please do not forget to complete the survey at the bottom of this page. Thank you for stopping by.
...
You can download the original [**_R Markdown_**](datatable.Rmd) and [**_R Code_**](datatable.R) of this tutorial.
...
Using basic exploratory analyses to:

1. Plot the trend for number of total and new cases in Coconino county and the state of Arizona and the US, 
2. Show which counties and which states have passed their COVID-19 peak.
3. Determine which counties and states have the highest number of cases per population. 


**Hint I**: You can obtain the main dataset containing number of COVID-19 cases in the US from [this link](https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_US.csv).

**Hint II**: You can obtain counties populaiton data from [this link](https://data.nber.org/census/popest/county_population.csv).
```{r}
library(data.table)
...
We need to download the population data and merge it with the COVID-19 data. However, this is tricky as we need a common column with matching values between the two tables. We can use the `Combined_Key` column from the `covid` and try to make a similar column in the population table that we are going to download.
```{r}
...
Now, we can merge the two datasets using the `area` column in `ppl_counties` and the `Combined_Key` column in `covid`. There might be few missing rows between the two, but for the purpose of this workshop, that is fine:
...
### Using Plotly
Using the `plotly` pachage in `R`, we can make beautiful ineractive plots. We start a simple but you can explore `plotly` functionalities further.
...
*Note:* As this part of the tutorial requires a running shiny server, we have set the code chunk as `echo=TRUE, eval=FALSE` and therefor the interface do not appear on our static HTML page. You can still run the Shiny interface on your personal computer, if you set the code chunk as `echo=FALSE, eval=TRUE` and add a new line in the header: `runtime: shiny`. 
...
```
## Excersie: COVID-19 per Area
Using the above datasets and data from [The United States Census Bureau](https://www.census.gov/), determine which counties have the highest number of COVID-19 cases per area. 
...
<iframe src="https://docs.google.com/forms/d/e/1FAIpQLScK9ZS8WhTCZ2BElAOy0QJu9m0ChBAbOqy0ektf62Q8eOL1Tw/viewform?embedded=true" width="800" height="1000" frameborder="0" marginheight="0" marginwidth="0">Loading…</iframe>
